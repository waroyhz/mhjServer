...
SP_Server::SP_Server(const char * bindIP, int port, SP_HandlerFactory * handlerFactory) {
...
int SP_Server::run() {
	...
	if (0 == ret) {
		...
		mIsRunning = 0;
		sp_syslog(LOG_WARNING, "Unable to create a thread for TCP server on port [%d], %s", mPort, strerror(errno));
	}
...
void SP_Server::pushResponseQueue(SP_Response* response) {
	msgqueue_push((struct event_msgqueue*) ((SP_EventArg*) peventArg)->getResponseQueue(), response);
}
